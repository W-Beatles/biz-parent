...
public class RedisHelper {
    ...
    public Boolean getLock(String lockName, String requestId, long milliseconds) {
        return stringRedisTemplate.opsForValue().setIfAbsent(lockName, requestId, milliseconds, TimeUnit.MILLISECONDS);
    }
    ...
    public Boolean getLock(String lockName, String requestId, long timeout, TimeUnit unit) {
        return stringRedisTemplate.opsForValue().setIfAbsent(lockName, requestId, timeout, unit);
    }
    ...
    public boolean delLock(String lockName, String requestId) {
        ...
        Long execute = stringRedisTemplate.execute(defaultRedisScript, keyList, requestId);
    ...
    public long getRedisLock(String lockName, long lockTimeout) {
        ...
        // 尝试获取锁
        Boolean getLock = stringRedisTemplate.opsForValue().setIfAbsent(lockName, String.valueOf(expireTime));
        if (Boolean.TRUE.equals(getLock)) {
            ...
            if (lockValue != null && System.currentTimeMillis() > Long.parseLong(lockValue)) {
                ...
                String getSetValue = stringRedisTemplate.opsForValue().getAndSet(lockName, String.valueOf(newExpireTime));
